...
Forwards through the LLM registry. **The `provider` field is a hint — the Bun service routes every id (`seher`/`default`/`claude`/`copilot`/`codex`) through the `seher-bridge` Rust binary**, which picks the actual agent at run time. See [design/llm-routing](/design/llm-routing/).
