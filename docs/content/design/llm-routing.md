...
SmartCrab does not bind to a single LLM provider. Instead, every LLM call funnels through `router.ts`, which delegates to [`seher-ts`](https://github.com/smartcrabai/seher-ts) — an external router SDK that resolves the highest-priority **available** coding agent at run time, given the user's settings file.

```
chat.bubble-send  ──┐
pipeline llm_call ──┤── router.route() ── seher-ts SeherSDK ──▶  Claude Code CLI
skill.invoke      ──┤                          │              │  Kimi CLI
memory.summarize  ──┘                          │              │  Copilot CLI
                                               │              │  Codex CLI
                                       fallback to            │
                                       llmRegistry.default()  ▼
                                       (the first registered  one chosen agent
                                        adapter — currently   actually invoked
                                        ClaudeLlmAdapter)
```

## Why one router, not many

`server.ts` registers every LLM provider id that nodes can mention — `seher`, `default`, `claude`, `kimi`, `copilot`, `codex` — against a **single bridge object**:
...
So a pipeline node that says `provider: claude` does **not** force the Claude SDK. seher-ts picks the actual agent at run time using priorities, time windows, and rate-limit state. The provider id in the YAML acts more like a hint or a documentation breadcrumb than a binding.
...
1. **Try seher-ts.** Lazy `await import("seher-ts")` (cached). If the import succeeds, instantiate `SeherSDK` with:
...
2. **Fall back to the registry.** If seher-ts is unavailable (not installed, import failure) or its `run()` throws, pick `llmRegistry.default()` — the first adapter registered, which today is `ClaudeLlmAdapter`. Use it directly with a single `user` message containing the prompt. Tag the response `kind: "registry-fallback"`.
3. **Hard error.** If neither path is available (no seher-ts and no registered LLM adapter), throw an explanatory error pointing the user at `~/.config/seher/settings.jsonc`.
...
`write-settings.ts:translateToSeherSettings` maps SmartCrab's in-app provider kinds to the agent / provider names seher-ts expects:

| SmartCrab `kind` | seher `command` | seher `provider.name` | seher `sdk` |
|------------------|-----------------|------------------------|-------------|
...
- `env`: the user's `envOverrides` (e.g. `ANTHROPIC_API_KEY=...`) if any
- `provider: { kind: "explicit", name }` for known kinds, otherwise `{ kind: "inferred" }`
- `sdk`: the matching SDK identifier so seher knows how to drive the CLI
...
GUI-launched apps on macOS inherit a minimal `PATH` that does not contain Homebrew, mise, or `~/.local/bin`. Without intervention, the embedded Bun service cannot find `claude`, `kimi`, or other CLIs that seher-ts dispatches to.

`BunServiceMacOS` works around this by spawning the user's login shell once at startup (`$SHELL -lc 'printf %s "$PATH"'`), capturing the output, and forwarding it to the child process's environment. The result is memoised because shell startup is non-trivial. This is what lets seher-ts succeed when it calls `Bun.which("claude")` from a Finder-launched app.

## Testing

Unit tests don't need a real seher install; they import `router.ts` and test the **fallback path** by registering a stub adapter into `llmRegistry`. End-to-end testing of routing requires either a real seher-ts settings file or a mock `SeherSDK`. The `optionalDependencies` block in `apps/bun-service/package.json` keeps `seher-ts` optional so CI without credentials still builds.
