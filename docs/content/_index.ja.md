...
SmartCrab は「ツール → AI」パラダイムを実現する macOS デスクトップアプリケーションです。HTTP リクエスト・cron ティック・チャットイベントといった非 AI 処理を先に走らせ、YAML で定義したパイプラインの条件分岐に基づいて AI エージェント（Anthropic / GitHub Copilot / OpenAI のいずれか。実行時に [`seher-bridge`](https://github.com/smartcrabai/seher) という Rust バイナリが解決し、Rust の `pi` エンジンで実行）を呼ぶかどうかを決めます。
...
# sakoku-ignore-next-line
| [llm-routing](/design/llm-routing/) | seher-bridge ルーターと Settings → `seher-config.yaml` の流れ |
# sakoku-ignore-next-line
...
# sakoku-ignore-next-line
| **seher-bridge** | ユーザー設定に基づいて最優先の利用可能エージェント（Anthropic / Copilot / OpenAI）を解決し、Rust の `pi` エンジンで実行する Rust バイナリ（`seher` クレートからビルドし `.app` に同梱）。Bun サービスとは stdio 上の NDJSON で通信する |
# sakoku-ignore-next-line
